...
import { getMBAEcoUnitByNumber, getMBAEcoUnitForTopicSlug, MBA_ECO_UNITS, unitRoman } from "@/lib/economicsUnits";
import {
  TrendingUp, FileText, HelpCircle, PlayCircle, BookOpen,
  ChevronRight, ScrollText, GraduationCap,
} from "lucide-react";

const MBAEconomicsPage = () => {
  ...
  const notesForUnit = useMemo(() =>
    (n: number) => notes.filter(x => getMBAEcoUnitForTopicSlug(x.topic_slug)?.number === n),
    [notes],
  );
  const quizzesForUnit = useMemo(() =>
    (n: number) => quizzes.filter(x => getMBAEcoUnitForTopicSlug(x.topic_slug)?.number === n),
    [quizzes],
  );
  ...
  const lecturesForUnit = useMemo(() =>
    (n: number) => lectures.filter((l: any) => getMBAEcoUnitForTopicSlug(l.topic_slug)?.number === n),
    [lectures],
  );
      ...
      <main id="main-content">
        ...
        <section className="border-b border-border bg-white">
          <div className="mx-auto max-w-5xl px-6 py-10">
            <Breadcrumbs items={[
              ...
              { label: "MBA Managerial Economics" },
            ]} />

            <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-accent-deep">MBA · PGDM · Semester 1</p>
        ...
        {failed && (
          <div className="mx-auto max-w-5xl px-6 pt-6">
            <ContentLoadError what="MBA Managerial Economics material" compact />
          </div>
        )}
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {MBA_ECO_UNITS.map(unit => {
                ...
                return (
                  <Link
                    key={unit.number}
                    to={`/mba-economics/unit-${unit.number}`}
        ...
        {loading ? (
          ...
          <div className="mx-auto max-w-5xl px-6 py-10 space-y-14">
            ...
            <section id="notes" className="scroll-mt-24">
              ...
              {notes.length === 0 ? (
                <EmptyState text="No notes uploaded yet for MBA Managerial Economics." />
              ) : (
            ...
            <section id="mcqs" className="scroll-mt-24">
              ...
              {quizzes.length === 0 ? (
                <EmptyState text="No MCQs uploaded yet for MBA Managerial Economics." />
              ) : (
                <div className="space-y-4">
                  {MBA_ECO_UNITS.map(unit => {
        ...
        <section className="border-t border-border bg-slate-50 py-10">
          <div className="mx-auto flex max-w-5xl flex-col items-start gap-4 px-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-lg font-bold text-foreground">Looking for BBA Business Economics?</h2>
              <p className="text-sm text-muted-foreground">We have a separate unit-wise hub for BBA students.</p>
            </div>
            <div className="flex flex-wrap gap-2.5">
              <Link to="/bba-economics" className="inline-flex items-center gap-1.5 rounded-md bg-accent px-4 py-2.5 text-sm font-semibold text-accent-foreground hover:brightness-110">
                <TrendingUp className="h-4 w-4" /> BBA Economics Hub
              </Link>
...
export default MBAEconomicsPage;
