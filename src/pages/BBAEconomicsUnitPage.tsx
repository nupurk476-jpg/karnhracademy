...
const BBAEconomicsUnitPage = () => {
  ...
  const unitNotes = useMemo(
    () => unit ? notes.filter(x => getBBAEcoUnitForTopicSlug(x.topic_slug)?.number === unit.number) : [],
    [notes, unit],
  );
  const unitQuizzes = useMemo(
    () => unit ? quizzes.filter(x => getBBAEcoUnitForTopicSlug(x.topic_slug)?.number === unit.number) : [],
    [quizzes, unit],
  );
  ...
  const unitLectures = useMemo(
    () => unit ? lectures.filter((l: any) => getBBAEcoUnitForTopicSlug(l.topic_slug)?.number === unit.number) : [],
    [lectures, unit],
  );
  ...
  if (!unit) {
    return (
      <div className="min-h-screen bg-background">
        <SEO title="Unit Not Found" description="This BBA Economics unit doesn't exist." path={`/bba-economics/unit-${unitNumber}`} noindex />
        <Header />
        <main id="main-content" className="mx-auto max-w-3xl px-6 py-24 text-center">
          ...
          <p className="mb-6 text-sm text-muted-foreground">
            BBA Business Economics only has Units I through V. Check the unit number in the URL, or browse the full hub.
          </p>
          <Link to="/bba-economics" className="inline-flex items-center gap-1.5 rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground hover:brightness-110">
            <ArrowLeft className="h-4 w-4" /> Back to BBA Economics
          </Link>
        </main>
        <Footer />
      </div>
  ...
  const prevUnit = getBBAEcoUnitByNumber(unit.number - 1);
  const nextUnit = getBBAEcoUnitByNumber(unit.number + 1);
  const roman = unitRoman(unit.number);

  return (
    <div className="min-h-screen bg-background">
      <SEO
        title={`BBA Business Economics Unit ${roman}: ${unit.title} — Notes, MCQs & Video Lectures`}
        description={`Unit ${roman} of BBA Business Economics — ${unit.title}. Study notes, MCQ practice sets and video lectures covering: ${unit.topics.map(t => t.label).join(", ")}.`}
        path={`/bba-economics/unit-${unit.number}`}
        jsonLd={{
          "@context": "https://schema.org",
          "@type": "LearningResource",
          name: `BBA Business Economics — Unit ${roman}: ${unit.title}`,
          description: `Unit ${roman} of BBA Business Economics — ${unit.topics.map(t => t.label).join(", ")}.`,
          about: unit.title,
          isPartOf: { "@type": "Course", name: "BBA Business Economics Study Hub", url: "https://karnhracademy.com/bba-economics" },
        ...
        <section className="border-b border-border bg-white">
          <div className="mx-auto max-w-4xl px-6 py-10">
            <Breadcrumbs items={[
              ...
              { label: "BBA Economics", to: "/bba-economics" },
              { label: `Unit ${roman}` },
            ]} />

            <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-accent-deep">BBA Business Economics · Unit {roman} of V</p>
            ...
            <p className="mb-5 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
              Notes, MCQ practice and video lectures for this unit of BBA Business Economics, organised by topic.
            </p>
        ...
        {loading ? (
          ...
          <div className="mx-auto max-w-4xl px-6 py-10 space-y-14">
            ...
            <nav className="flex items-center justify-between gap-3 border-t border-border pt-8">
              {prevUnit ? (
                <Link to={`/bba-economics/unit-${prevUnit.number}`} className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-accent-deep">
                  <ArrowLeft className="h-4 w-4" /> Unit {unitRoman(prevUnit.number)}: {prevUnit.title}
                </Link>
              ) : <span />}
              <Link to="/bba-economics" className="text-sm font-medium text-accent-deep hover:underline">
                All Units
              </Link>
              {nextUnit ? (
                <Link to={`/bba-economics/unit-${nextUnit.number}`} className="flex items-center gap-2 text-right text-sm font-medium text-muted-foreground hover:text-accent-deep">
...
export default BBAEconomicsUnitPage;
