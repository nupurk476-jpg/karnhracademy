  ...
  const body = (
    <>
      <div className="relative">
        <SubjectCover id={d.value} code={meta.code} caption={meta.exams.join(" · ")} icon={d.icon} image={meta.image} className="rounded-t-2xl border-b border-slate-200" />
        <span
          className="absolute left-3 top-3 rounded-md px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-wider"
          style={comingSoon ? { background: BRAND.navy, color: "#FFFFFF" } : { background: BRAND.gold, color: "#FFFFFF" }}
        >
